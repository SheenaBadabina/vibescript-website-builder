...
export async function getUser(env, email){ return json(await env.USERS.get(`user:${email}`)); }
...
export async function takeToken(env, token){
  const key = `token:${token}`; const v = await env.VERIFY.get(key); if(!v) return null; await env.VERIFY.delete(key); return JSON.parse(v);
}
...
function json(v){ return v ? JSON.parse(v) : null; }
