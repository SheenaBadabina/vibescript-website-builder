...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (mode === "create") {
      ...
      if (existing) return json({ ok: false, error: "Email already in use" }, 409);

      const salt = crypto.getRandomValues(new Uint8Array(16));
      const hash = await hashPass(password, salt);
      const user = { email: e, pass: b64(hash), salt: b64(salt), verified: false, createdAt: Date.now() };
...
async function hashPass(pass, salt){ const enc=new TextEncoder(); const data=new Uint8Array([...salt, ...enc.encode(pass)]); const h=await crypto.subtle.digest("SHA-256", data); return new Uint8Array(h); }
async function verifyPass(pass, salt, target){ const h=await hashPass(pass, salt); if(h.length!==target.length) return false; let d=0; for(let i=0;i<h.length;i++) d|=h[i]^target[i]; return d===0; }
async function sendVerification(env, email){
  const token = crypto.randomUUID().replace(/-/g,"") + crypto.randomUUID().slice(0,8);
  await env.VERIFY.put(token, JSON.stringify({ email }), { expirationTtl: 600 });
