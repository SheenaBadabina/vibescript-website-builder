...
// Checks verification; unverified users get a prompt to resend link.
...
export async function onRequestPost({ request, env }) {
  ...
  if (!user) return html(400, errorPage("Account not found. Please sign up.", true));
  if (user.password !== password) return html(400, errorPage("Incorrect password.", false));
...
function errorPage(msg, offerSignup=false){
  return page("Sign in error", `
    ...
    <div class="flex gap-2">
      ...
      ${offerSignup ? `<a href="/signup" class="rounded-lg bg-gradient-to-r from-pink-500 via-yellow-400 to-teal-400 px-4 py-2 text-sm font-semibold text-black">Create account</a>` : ``}
    </div>
  `);
}
function verifyNeeded(email){
  return page("Email not verified", `
    <h1 class="text-xl font-bold text-white mb-2">Confirm your email</h1>
    <p class="text-sm text-gray-300 mb-4">We still need to verify <span class="font-semibold">${escapeHtml(email)}</span>.</p>
    <form method="POST" action="/api/resend-confirmation" class="space-y-4">
